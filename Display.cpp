...
extern void updateActiveWindow(bool full);
...
COLD void displayFreq(void)
{
	...
	if (popup) return;   // Do not write to the screen when a control window is active
...
COLD void displayXVTR()
{
	...
	if (bandmem[curr_band].bandmap_en && bandmem[curr_band].xvtr_IF)   //bandmem[curr_band].xvtr_num)
		xvtr = 1;
	else
		xvtr = 0;
	#ifndef USE_RA8875
...
COLD void displayMenu() 	{if (popup) return; draw_2_state_Button(MENU_BTN, &std_btn[MENU_BTN].enabled);				 }
